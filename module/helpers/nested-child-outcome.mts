...
export function summarizeNestedChildOutcome(childState: unknown, childResult: unknown = null) {
  const child = record(childState);
  const result = record(childResult);
  const status = text(child.status) ?? text(result.status);
  if ( status !== "failed" && status !== "cancelled" && result.ok !== false ) return null;
...
function entries(value: unknown): Data[] {
...
function diagnostic(value: Data) {
