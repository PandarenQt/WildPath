...
import { movementTransitionRelations } from "../helpers/movement-transition.mjs";
import { createResolutionState, updateResolutionState, runResolutionPipeline, resumeResolutionPipeline, continueResolutionStage, completeResolutionStage, failResolutionStage, failResolutionState } from "../helpers/resolution-state.mjs";
...
export function createMovementResolutionHost(options, ports) {
    ...
    const initial = { completedTransitionCount: 0, cumulativeCost: 0, stopped: false, reason: null };
    ...
    const stages = [];
    const stage = (id, run, active = false) => {
        stages.push({ id, run, metadata: {}, ...(active ? { canRun: (s) => !movementTraversal(s).stopped } : {}) });
    };
    ...
    stage("movement.validation", s => {
        if (!evaluation.valid)
            return failResolutionStage({ state: s, code: "MOVEMENT_ROUTE_INVALID",
                reason: "Movement route is invalid.", data: { failures: evaluation.failures } });
        const validity = ports.validate(s, movementTraversal(s));
        if (validity.decision !== "continue")
            return failResolutionStage({ state: s, code: "MOVEMENT_STATE_INVALID",
    ...
    for (let index = 0; index < count; index++) {
        ...
        stage(`${prefix}.propose`, s => {
            ...
            if (traversal.completedTransitionCount !== index)
                throw new Error("Movement traversal cursor is out of order.");
        ...
        const reaction = createReactionWindowStage({ id: `${prefix}.reactions`, timing: "before-transition",
            eventSelector: s => proposedEvent(s), discovery: context => {
                if (ports.discovery)
                    return ports.discovery(context);
                const reactions = context.services.reactions;
        ...
        stage(`${prefix}.revalidation`, s => {
            ...
            if (validity.decision === "invalid")
                return failResolutionStage({ state: s, code: "MOVEMENT_STATE_STALE",
            ...
            if (validity.decision === "stop" || !payable)
                return continueResolutionStage({ state: withTraversal(s, { ...traversal,
    ...
    stage("movement.payment", s => {
        ...
        if (!payment.ok)
            return failResolutionStage({ state: s, code: "MOVEMENT_PAYMENT_UNAVAILABLE",
    ...
    let execution;
    return { state,
        plan: ({ state: current, services = {} }) => runResolutionPipeline({ state: current, stages, services }),
        resume: ({ state: current, response, services = {} }) => {
            const validity = ports.validate(current, movementTraversal(current));
            if (validity.decision === "invalid")
                return { ok: false, state: failResolutionState(current, { code: "MOVEMENT_STATE_STALE", reason: validity.reason ?? "Movement source changed while awaiting a response." }) };
        ...
        completeChild: (args) => {
            ...
            if (current.status === "cancelled") {
                ...
                const validity = ports.validate(current, movementTraversal(current));
                if (validity.decision === "invalid")
                    return { ...completed, ok: false, state: failResolutionState(current, { code: "MOVEMENT_STATE_STALE", reason: validity.reason ?? "Movement source changed during reaction." }) };
                if (validity.decision === "stop")
                    current = withTraversal(current, { ...movementTraversal(current),
        ...
        execute: async ({ state: current, authority }) => {
    ...
    async function commit(current) {
        const validity = ports.validate(current, movementTraversal(current));
        if (validity.decision === "invalid" || (validity.decision === "stop" && !movementTraversal(current).stopped))
            return { ok: false, state: failResolutionState(current, {
        ...
        const payment = planPayment(evaluation.cost.consumesBudget ? traversal.cumulativeCost : 0);
        if (!payment.ok)
            return { ok: false, state: failResolutionState(current, { code: "MOVEMENT_PAYMENT_UNAVAILABLE",
        ...
        if (!transaction.ok)
            return { ok: false, state: failResolutionState(current, { stageId: "movement.commit",
    ...
    function footprint(index) {
        ...
        if (!value)
            throw new Error("Movement cursor has no tactical footprint.");
        return value;
    }
}
export function movementTraversal(state) {
