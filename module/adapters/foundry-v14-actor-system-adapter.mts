...
export function foundryActorSystemSnapshot(actor: ActorSourceDocument): Record<string, unknown> {
  try {
    if ( typeof actor?.toObject !== "function" ) throw new TypeError("Actor.toObject(true) is required.");
...
function projectEffectiveResources(snapshot: Record<string, unknown>, system: unknown): void {
  if ( snapshot.resources === undefined && snapshot.pools === undefined ) return;
  ...
  if ( snapshot.pools !== undefined ) {
    ...
    const effectivePools = new Map<string, Record<string, unknown>>();
    ...
    for ( const entry of snapshot.pools ) {
      ...
      if ( typeof pool.id !== "string" || !pool.id ) throw new TypeError("Source pool ID is required.");
      projectResource(pool, effectivePools.get(pool.id), `pools.${pool.id}`);
    }
  }
}

function projectResource(source: unknown, prepared: unknown, path: string): void {
  ...
  for ( const key of ["value", "max"] as const ) {
    ...
    if ( !(key in resource) && !(key in effective) ) continue;
