...
export function stagedMovementPersistence(base: DocumentPersistencePort, token: Token, id: string,
  canCommit: () => boolean): DocumentPersistencePort {
  return {...base, async updateActor(input) {
    if (!canCommit()) throw new Error("Staged movement payment requires current GM authority.");
    return base.updateActor(input);
  }, async updateDocument(input) {
    if (input.document !== token || !canCommit()) throw new Error("Staged movement position requires current GM authority.");
    if (writes.has(token)) throw new Error("A staged position write is already in flight.");
    ...
    try {
      ...
      const write = (values: Data) => base.updateDocument({...input, updates: values, operation: {animate: false,
