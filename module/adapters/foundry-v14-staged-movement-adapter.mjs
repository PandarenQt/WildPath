import { createMovementResolutionHost } from "../resolvers/movement-pipeline-resolver.mjs";
...
export async function foundryMovementIntentToStagedOptions({ intent, resolutionId, senderUserId, game, persistencePort }) {
    ...
    if (!canCommit())
        throw new Error("Staged movement requires an active GM.");
    ...
    if (!documents.ok)
        throw new Error(documents.reason ?? "Movement documents could not be resolved.");
    ...
    if ((intent.movementKind ?? "voluntary") !== "voluntary")
        throw new Error("Staged Foundry movement currently requires voluntary translation.");
    const translated = foundryMovementIntentToMovementPath({ intent, tokenDocument: token, scene });
    if (!translated.ok)
        throw new Error(translated.reason ?? "Movement route could not be translated.");
    ...
    const actorDocuments = Object.fromEntries(actors.flatMap(a => {
        const aliases = [[a.uuid, a]];
        if (actors.filter(other => other.id === a.id).length === 1)
            aliases.push([a.id, a], [`actor:${a.id}`, a]);
        return aliases;
    }));
    for (const t of [token, ...observers.map(o => o.token)]) {
        for (const ref of [t.uuid, `uuid:${t.uuid}`, `token:${t.id}`])
            actorDocuments[ref] = t.actor;
    }
    // Ambiguous base Actor IDs are omitted. Distinct synthetic Actors must use UUIDs in reaction definitions.
    const controllers = Object.fromEntries(Object.entries(actorDocuments).map(([key, value]) => [key, [...game.users.values()].filter(user => user.active && !user.isGM && value.testUserPermission(user, "OWNER")).map(user => user.id)]));
    for (const ids of Object.values(controllers))
        if (!ids.length)
    ...
    const services = { ...supplied, targetActors: { ...actorDocuments }, reactions: { ...supplied.reactions,
            ...
            createChildState: (args) => {
                ...
                if (matches.length !== 1)
                    throw new Error("Reaction must resolve to exactly one configured observer Token.");
                const reactor = matches[0].token;
                const definition = candidate.actionDefinition ?? candidate.action;
                if (!definition)
                    throw new Error("Movement reaction requires an ActionDefinition.");
                const proposed = args.parentState.results.proposedMovement;
                ...
                if (!targetResult.ok || !("tokenFootprint" in targetResult) || !targetResult.tokenFootprint)
                    throw new Error("Logical movement target footprint is invalid.");
                const sourceResult = adapter.tokenToFootprint(reactor, { position: reactor.toObject(true) });
                if (!sourceResult.ok || !("footprint" in sourceResult) || !sourceResult.footprint)
                    throw new Error("Reaction source footprint is invalid.");
    ...
    const host = createMovementResolutionHost({ id: resolutionId, path: translated.path, source,
        ...
        observers: () => observers.map(observer => {
            if (observer.token.parent !== scene)
                throw new Error("Movement observer must remain on the same Scene.");
            const result = adapter.tokenToFootprint(observer.token, { position: observer.token.toObject(true) });
            if (!result.ok || !("footprint" in result) || !result.footprint)
                throw new Error("Movement observer footprint is invalid.");
            // Legacy JS widens the canonical footprint's literal type; success was checked above.
            return { id: observer.id, footprint: result.footprint, reachFields: observer.reachFields, context: observer.context ?? {} };
        }),
        validate: (state, traversal) => {
            if (!canCommit())
                return { decision: "invalid", reason: "Movement authority changed." };
    ...
    function positionUpdates(footprint) {
        ...
        if (!from.ok || !to.ok || !("point" in from) || !("point" in to))
            throw new Error("Cannot translate tactical anchor to Foundry position.");
